package com.kazumaproject.symbol_keyboard.emoji_kitchen
...
object EmojiKitchenManager {
    ...
    fun loadMetadata(context: Context) {
        ...
        try {
            ...
            isLoaded = true
            Log.d("EmojiKitchenManager", "メタデータのロードに成功しました。絵文字数=${emojiList.size}, 組み合わせ数=${combinations.size}")
        } catch (e: Exception) {
            Log.e("EmojiKitchenManager", "メタデータのロードに失敗しました。", e)
        }
    }
     ...
     */
    suspend fun downloadSticker(urlString: String): Bitmap? {
        return null
    }
